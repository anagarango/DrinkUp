...
export default function Home() {
  ...
  const GetCocktail = async () => {
    ...
    .then((results) => {
        ...
        setCocktails("");
        setNewCocktails()
        function CheckingCategoryAlcohol(){
          var matchCategoryAlcohol = [];
          ...
          for(var x = 0; x < drinks.length; x++){
            if(drinks[x].strCategory == category && drinks[x].strAlcoholic == alcoholic){
              matchCategoryAlcohol.push(drinks[x])
            }
          }
          return matchCategoryAlcohol
        }
          
        
        function CheckingCocktailIngredientMatch(callback){
          var matchCocktail = []
          for(var i = 1; i <= 15; i++){
            if(callback[x]['strIngredient'+i] == ingredients){
              matchCocktail.push(callback[x])
            }
          }
        ...
        setTimeout(() => {
          ...
          console.log(CheckingCocktailIngredientMatch(CheckingCategoryAlcohol))
          setNewCocktails(CheckingCocktailIngredientMatch(CheckingCategoryAlcohol))
          setLoading(false)
        }, 1500);
    ...
    <>
      ...
      <main>
        ...
        <FlexBox bgImage="/SecondHero.jpeg" minHeight="400px" height="50vw" width="100vw" dir="column" justifyContent="flex-end">
          <Heading padBod="35px">Feel Good About What You Drink</Heading>
